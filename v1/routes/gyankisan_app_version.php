<?php 
        ...
        class Db_app_version
        {
            ...
            {
                $stmt = $this->conn->prepare("SELECT * FROM tbl_gyan_app_version ORDER BY id DESC LIMIT 1 "); 
                $stmt->execute(); 
                $row = $stmt->fetch();
                $version  = $row['version'] ;
        ...
        $app->post('/gyan_app_version', function() use ($app){
